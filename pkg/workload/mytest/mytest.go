...
const (
	mytestSchema = `(
		k INT8 NOT NULL DEFAULT unordered_unique_rowid(),
        v BYTES NOT NULL,
        CONSTRAINT mytest_pkey PRIMARY KEY (k ASC)
	)`
	mytestSchemaWithIndex = `(
...
func init() {
	workload.Register(mytestMeta)
}

var mytestMeta = workload.Meta{
	Name:        `mytest`,
	...
	New: func() workload.Generator {
		g := &mytest{}
		g.flags.FlagSet = pflag.NewFlagSet(`mytest`, pflag.ContinueOnError)
...
func (*mytest) Meta() workload.Meta { return mytestMeta }

// Flags implements the Flagser interface.
func (w *mytest) Flags() workload.Flags { return w.flags }

// Hooks implements the Hookser interface.
func (w *mytest) Hooks() workload.Hooks {
...
ALTER TABLE mytest ADD COLUMN e enum_type NOT NULL AS ('v') STORED;`)
			return err
		},
		Validate: func() error {
...
func (w *mytest) Tables() []workload.Table {
	...
	if w.shards > 0 {
		schema := shardedMytestSchema
		if w.secondaryIndex {
			schema = shardedMytestSchemaWithIndex
		}
		...
		if w.secondaryIndex {
			table.Schema = mytestSchemaWithIndex
		} else {
			table.Schema = mytestSchema
		}
	}
...
func (w *mytest) Ops(
	...
	if w.shards == 0 {
		buf.WriteString(`SELECT k, v FROM mytest WHERE k IN (`)
		...
		buf.WriteString(`SELECT k, v, e FROM mytest WHERE k IN (`)
		for i := 0; i < w.batchSize; i++ {
			if i > 0 {
				buf.WriteString(", ")
			}
		...
		buf.WriteString(`SELECT k, v FROM mytest WHERE (shard, k) in (`)
		for i := 0; i < w.batchSize; i++ {
			if i > 0 {
				buf.WriteString(", ")
			}
	...
	// Write statement
	buf.Reset()
	buf.WriteString(`INSERT INTO mytest (v) VALUES`)
	for i := 0; i < w.batchSize; i++ {
		if i > 0 {
			buf.WriteString(", ")
		}
	...
	if w.writesUseSelectForUpdate {
		if w.shards != 0 {
			return workload.QueryLoad{}, fmt.Errorf("select for update in mytest requires shard=0")
		}
		buf.Reset()
		buf.WriteString(`SELECT k, v FROM mytest WHERE k IN (`)
		for i := 0; i < w.batchSize; i++ {
	...
	// Span statement
	buf.Reset()
	buf.WriteString(`SELECT count(v) FROM [SELECT v FROM mytest`)
	if w.spanLimit > 0 {
	...
	for i := 0; i < w.connFlags.Concurrency; i++ {
		...
		if err := op.sr.Init(ctx, "mytest", mcp, w.connFlags); err != nil {
			return workload.QueryLoad{}, err
		}
		op.mcp = mcp
		if w.sequential {
...
func (o *mytestOp) run(ctx context.Context) (retErr error) {
	...
	for i := 0; i < o.config.batchSize; i++ {
		...
		// }
		writeArgs[i] = randomBlock(o.config, o.g.rand())
	}
	start := timeutil.Now()
	var err error
	if o.config.writesUseSelectForUpdate {
...
func (o *mytestOp) tryHandleWriteErr(name string, start time.Time, err error) error {
...
func (o *mytestOp) close(context.Context) {
...
func randomBlock(config *mytest, r *rand.Rand) []byte {
