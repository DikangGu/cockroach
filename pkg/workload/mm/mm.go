...
func init() {
	workload.Register(mmMeta)
}

var mmMeta = workload.Meta{
	Name:        `mm`,
	Description: `This simulates the matchmaking workload at Roblox`,
	Details: `
	TBD
	`,
	Version:      `1.0.0`,
	...
	New: func() workload.Generator {
		g := &mm{}
		g.flags.FlagSet = pflag.NewFlagSet(`mm`, pflag.ContinueOnError)
...
func (*mm) Meta() workload.Meta { return mmMeta }

// Flags implements the Flagser interface.
func (w *mm) Flags() workload.Flags { return w.flags }

// Hooks implements the Hookser interface.
func (w *mm) Hooks() workload.Hooks {
...
ALTER TABLE mm ADD COLUMN e enum_type NOT NULL AS ('v') STORED;`)
			return err
		},
		Validate: func() error {
...
func (w *mm) Tables() []workload.Table {
	table := workload.Table{
		Name: `mm`,
		// TODO(dan): Support initializing mm with data.
	...
	if w.shards > 0 {
		...
		if w.secondaryIndex {
			table.Schema = mmSchemaWithIndex
		} else {
			table.Schema = mmSchema
		}
	}
...
func (w *mm) Ops(
	...
	if w.shards == 0 {
		buf.WriteString(`SELECT k, v FROM mm WHERE k IN (`)
		...
		buf.WriteString(`SELECT k, v, e FROM mm WHERE k IN (`)
		for i := 0; i < w.batchSize; i++ {
			if i > 0 {
				buf.WriteString(", ")
			}
		...
		buf.WriteString(`SELECT k, v FROM mm WHERE (shard, k) in (`)
		for i := 0; i < w.batchSize; i++ {
			if i > 0 {
				buf.WriteString(", ")
			}
	...
	// Write statement
	buf.Reset()
	buf.WriteString(`UPSERT INTO mm (k, v) VALUES`)
	for i := 0; i < w.batchSize; i++ {
		j := i * 2
		if i > 0 {
			buf.WriteString(", ")
	...
	if w.writesUseSelectForUpdate {
		if w.shards != 0 {
			return workload.QueryLoad{}, fmt.Errorf("select for update in mm requires shard=0")
		}
		buf.Reset()
		buf.WriteString(`SELECT k, v FROM mm WHERE k IN (`)
		for i := 0; i < w.batchSize; i++ {
	...
	// Span statement
	buf.Reset()
	buf.WriteString(`SELECT count(v) FROM [SELECT v FROM mm`)
	if w.spanLimit > 0 {
	...
	for i := 0; i < w.connFlags.Concurrency; i++ {
		...
		if err := op.sr.Init(ctx, "mm", mcp, w.connFlags); err != nil {
			return workload.QueryLoad{}, err
		}
		op.mcp = mcp
		if w.sequential {
...
func (o *mmOp) run(ctx context.Context) (retErr error) {
...
func (o *mmOp) tryHandleWriteErr(name string, start time.Time, err error) error {
...
func (o *mmOp) close(context.Context) {
...
func randomBlock(config *mm, r *rand.Rand) []byte {
